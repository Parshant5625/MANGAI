...
class LandsatSurfaceTemperatureFusion:
        ...
        *,
        ...
        try:
            if aoi_bbox is not None and href.startswith(("http://", "https://")):
                thermal_values, uncertainty_k, qa_rejected = self._sample_remote_cog(
                    href, optical_batch, aoi_bbox, assets
                )
            else:
        ...
        for record, temperature_c, is_valid, uncertainty in zip(
            optical_batch.records, temperatures_c, valid, uncertainty_k
        ):
            if not is_valid:
                continue
            updated = dict(record)
        ...
        if records and any("thermal_uncertainty_k" in record for record in records):
            uncertainty_values = np.array(
                [record["thermal_uncertainty_k"] for record in records], dtype=np.float32
            )
            uncertainty_quality = np.clip(
                1.0 - uncertainty_values / MAX_ST_UNCERTAINTY_K, 0.0, 1.0
            ).mean()
    ...
    def _sample_remote_cog(
        ...
        if not qa_pixel_href or not st_qa_href:
            raise DataUnavailableError(
                "Landsat ST scene is missing required QA_PIXEL or ST_QA assets.",
                details={"required_assets": ["qa_pixel", "st_qa"]},
            )
    ...
    def _sample_window(array: Any, transform_: Any, window: Any, xs: list[float], ys: list[float]) -> np.ndarray:
        ...
        for index, (x, y) in enumerate(zip(xs, ys)):
            ...
            value = LandsatSurfaceTemperatureFusion._value_from_array(
                array, int(row) - int(window.row_off), int(col) - int(window.col_off)
            )
            if value is not None:
                values[index] = value
        return values
    ...
    def _value_from_dataset(dataset: Any, row: int, col: int) -> float | None:
        ...
        return LandsatSurfaceTemperatureFusion._value_from_array(block, 0, 0)
