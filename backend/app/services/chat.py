...
class ChatService:
    """Deterministic, evidence-first assistant over MANGAI's existing intelligence services."""
    ...
    @staticmethod
    def _actions(*items: tuple[str, str]) -> list[dict[str, str]]:
        return [{"label": label, "action": action} for label, action in items]

    def answer(self, message: str, site_id: str | None = None, page_context: str | None = None) -> dict[str, Any]:
        text = message.strip()
        common = {
            "data_mode": self.settings.data_mode,
            "synthetic_data": self.settings.data_mode == "demo",
            ...
            "actions": self._actions(("Open Overview", "overview"), ("Open Production", "production"), ("Open Reserve", "reserve"), ("Open Actions", "recommendations")),
        }
        if self._match(text, ("hello", "hi", "hey", "help", "what can you do")):
            return {**common, "intent": "help", "confidence": 0.99, "answer": "I am MANGAI AI — an evidence-first mining intelligence copilot. I can investigate production risk, reserve prospectivity, fleet downtime, weather and blasting conditions, recommendations, and model/data health. I can also take you directly to the relevant dashboard view.", "evidence": [], "suggested_questions": self.suggestions()}
        ...
        if self._match(text, ("recommendation", "recommend", "action", "what should", "what do we do", "next action")):
            ...
            return {**common, "intent": "recommendations", "confidence": 0.93, "answer": f"MANGAI has {len(items)} recommendation(s) available. Highest-confidence actions include: {titles or 'no active recommendations returned'}. These are decision-support suggestions and require human approval before execution.", "evidence": [{"label": "Active recommendations", "value": str(len(items))}] + [{"label": "Action", "value": str(item.get("title", "Recommendation"))} for item in top], "suggested_questions": ["Why is production risk high?", "Which equipment needs attention first?", "What is the reserve priority area?"], "actions": self._actions(("Open Action Center", "recommendations"), ("Open Operations", "operations"))}
        if self._match(text, ("health", "model health", "drift", "data quality", "model status")):
            monitoring = ModelMonitoringService().health()
            quality = DataQualityService().run()
            quality_value = quality.get("overall_score", quality.get("score", "N/A"))
            return {**common, "intent": "health", "confidence": 0.9, "answer": f"MANGAI is running in {self.settings.data_mode.upper()} mode. Model monitoring status is {monitoring.get('status', 'UNKNOWN')} and the latest data-quality score is {quality_value}. Open the Health page for detailed diagnostics.", "evidence": [{"label": "Data mode", "value": self.settings.data_mode.upper()}, {"label": "Monitoring", "value": str(monitoring.get("status", "UNKNOWN"))}, {"label": "Data quality", "value": str(quality_value)}], "suggested_questions": ["What is the production risk?", "What reserve targets are strongest?", "What can you do?"], "actions": self._actions(("Open Model Health", "health"), ("Open Data Health", "health"))}
        return {**common, "intent": "unknown", "confidence": 0.55, "answer": "I can investigate reserve prospectivity, production forecasts and shortfall risk, equipment, weather/blasting, recommendations, and model/data health. Try one of the suggested questions below.", "evidence": [], "suggested_questions": self.suggestions()}
